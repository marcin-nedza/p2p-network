...
//    }
//
//    public void handleMessage(PeerMessage rpc) {
//        String from = rpc.from();
    ...
    public void shutdown() {
        ...
        if (serverSocket != null && !serverSocket.isClosed()) {
            try {
                ...
                logErr("Failed to close server socket: " + e.getMessage());
            }
        }
    }
    ...
    private void printPeersState(String context) {
        ...
        if (peers.isEmpty()) {
            ...
            peers.forEach((peerId, socket) -> {
                String direction = (socket.getPort() == opts.getListenPort()) ? "INCOMING" : "OUTGOING";
                System.out.printf("- [%s] %s:%d (local: %d) [%s]%n",
                        peerId,
